...
$$\theta_i (s) = \rho_1*\tau_{EDC} (s)/ I_{i} +\rho_2*\tau_{FDP} (s)/ I_{i} + \rho_3*\tau_{FDS} (s)  / I_{i} -\theta_{i}(0)*(s+B_i/I_i)/(s^2+B_i s/I_i+K_i/I_i)+K_i*\theta_{i,eq}/(I_i*(s^2+B_i s/I_i+K_i/I_i)$$
